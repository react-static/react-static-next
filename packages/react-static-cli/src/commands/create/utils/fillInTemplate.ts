...
interface FillInTemplateResult extends Readonly<FillInTemplateResult> {
  path: string
  messages: readonly ReportableMessage[]
}

function guardIsLocal(
  values: TemplateOptions
): values is TemplateOptions & { isLocal: true; localPath: string } {
...
export async function fillInTemplate(
  ...
  if (guardIsLocal(values)) {
    ;["dependencies", "devDependencies"].forEach(group => {
      ...
      messages.push(
        ...linkLocalDependencies(packageJson[group], resolutions, {
...
function linkDependency(
  packageName: string,
  packagePath: string,
  { cwd, packageManager }: { cwd: string; packageManager: "yarn" | "npm" }
): void {
