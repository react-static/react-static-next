...
export function SelectTemplate({ onSelect }) {
    const handleSelect = ({ value }) => {
        onSelect(value);
    };
    // Get description column size
    const descriptionColumn = useMemo(() => Math.max(...FEATURED_TEMPLATES.map(t => typeof t === "object" ? t.shortName.length : 0)) + 2, []);
    const columns = (process.stdout.columns || 80) - "package manager".length - 2;
    // Wrap text
    const wrap = useMemo(() => wordwrap(descriptionColumn + 2, columns), [
        descriptionColumn
    ]);
    const items = useMemo(() => FEATURED_TEMPLATES.map(template => {
        const label = padEnd(template.shortName, descriptionColumn) +
